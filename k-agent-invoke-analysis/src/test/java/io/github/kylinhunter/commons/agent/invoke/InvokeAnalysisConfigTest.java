...
class InvokeAnalysisConfigTest {
    ...
    void getConfig() {
        ...
        InvokeAnalysisConfig pluginConfig = pluginConfigReader.read(InvokeAnalysisConfig.class, "invoke-analysis");
        System.out.println(pluginConfig);

        Assertions.assertEquals("io.github.kylinhunter.commons.agent.invoke.test",
                pluginConfig.getPoints().get(0).getType().getNameStartsWith());
        Assertions.assertEquals("doHomeWork",
                pluginConfig.getPoints().get(0).getMethod().getNameContains());

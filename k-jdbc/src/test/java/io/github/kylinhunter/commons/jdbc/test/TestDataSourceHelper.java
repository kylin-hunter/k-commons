...
public class TestDataSourceHelper {
  ...
  private static void mockTableMetadata(DatabaseMetaData databaseMetaData) throws SQLException {
    ...
    mockTableMetadata(databaseMetaData, TestHelper.MONITOR_SCAN_TASK);
  ...
  private static void mockColumnMetadata(DatabaseMetaData databaseMetaData) throws SQLException {
    ...
    Mockito.when(databaseMetaData.getColumns(any(), eq(null), eq(TestHelper.TEST_TABLE_ROLE1),
    ...
    Mockito.when(getTablesResultSet1.getObject(1)).thenReturn(TestHelper.TEST_TABLE_ROLE1);

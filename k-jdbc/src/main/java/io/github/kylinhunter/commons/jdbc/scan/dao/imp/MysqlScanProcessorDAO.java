 ...
 */
public class MysqlScanProcessorDAO extends AbstractDatabaseManager implements
    ScanProcessorDAO {

  private static final String INIT_SQL = "io/github/kylinhunter/commons/jdbc/scan/scan-mysql-scan_processor.sql";

  private static final String INSERT_SQL = "insert into %s"
      + " (id, data_id, op,status) values(?,?,?,?)";

  private static final String DELETE_SQL = "delete from %s  where id=?";
  private static final String UPDATE_SQL = "update %s"
      + " set op=?,status=? where id=? and data_id=?";


  private static final String SELECT_SQL = "select  id,  data_id as dataId, op ,status from %s"
  ...
  public void save(String tableName, ScanProcessor scanProcessor) {
    ...
    this.getSqlExecutor().execute(sql, scanProcessor.getId(), scanProcessor.getDataId(),
        scanProcessor.getOp(), scanProcessor.getStatus());
  ...
  public void update(String tableName, ScanProcessor scanProcessor) {
    ...
    this.getSqlExecutor().execute(sql,
        scanProcessor.getOp(), scanProcessor.getStatus(), scanProcessor.getId(),

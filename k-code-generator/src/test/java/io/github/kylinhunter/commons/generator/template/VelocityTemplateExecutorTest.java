...
class VelocityTemplateExecutorTest {
  ...
  void testTemplateInClass() {
    String templateInClass =
        "io/github/kylinhunter/commons/generator/template/template_in_class.vm";
  ...
  void testTemplateInFile() {
    ...
    templateEngine.customize(
        config -> {
          ...
          config.setTemplatePath(UserDirUtils.getDir("src/main/resources/templates").toPath());
        });
    templateExecutor
        .tmplate(templateInFile, null, FileExtensions.VM)
        .outputRelativePath("output3_result1.html")
        .encoding("UTF-8")
        .build();
    templateExecutor.output(System.out::println);
